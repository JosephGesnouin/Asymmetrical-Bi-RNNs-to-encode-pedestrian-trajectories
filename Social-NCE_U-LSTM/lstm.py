...
class LSTM(torch.nn.Module):
    ...
    def step(self, lstm, hidden_state, obs1, obs2, goals, batch_split, hidden_only = False, h_inv = None, no_pool=False, ln=None):
    ...
    def forward(self, observed, goals, batch_split, prediction_truth=None, n_predict=None):
        ...
        prediction_truth = copy.deepcopy(list(itertools.chain.from_iterable(
            (observed[-2:], prediction_truth)
                ...
                # ----------- Social NCE -------------
        hidden_state_memory = []
        hidden_state_memory.append(torch.stack([h for h in hidden_state[0]], dim=0))
