...
class LSTM(torch.nn.Module):
    def __init__(self, embedding_dim=64, hidden_dim=128, pool=None, pool_to_input=True, goal_dim=None, goal_flag=False):
        ...
        self.encoder2 = torch.nn.GRUCell(self.embedding_dim, self.hidden_dim)
        self.decoder = torch.nn.GRUCell(self.embedding_dim + pooling_dim, 2*self.hidden_dim)
        ...
        self.hidden2normal = Hidden2Normal(2*self.hidden_dim)
    ...
    def forward(self, observed, goals, batch_split, prediction_truth=None, n_predict=None):
        ...
        h_forward = [torch.zeros(self.hidden_dim, device=observed.device) 
        ...
        for obs1, obs2 in zip(inv_observed[1:], inv_observed[:-1]):
            ##LSTM Step
            h_forward = self.step(self.encoder1, h_forward, obs1, obs2, goals, batch_split, 
                            hidden_only=True, no_pool=True)#, ln=self.ln_e1)

        h_backward = [torch.zeros(self.hidden_dim, device=observed.device) 
                                for _ in range(num_tracks)]

        for obs1, obs2 in zip(observed[:-1], observed[1:]):
            ##LSTM Step
            h_backward = self.step(self.encoder2, h_backward, obs1, obs2, goals, batch_split, 
                            hidden_only=True, no_pool=True)#, ln=self.ln_e2)
            # concat predictions
            #!!!normals.append(normal)
            #!!!positions.append(obs2 + normal[:, :2])  # no sampling, just mean

        hidden_state = [torch.cat((hf,hb)) for hf, hb in zip(h_forward, h_backward)]
        #hidden_state = [torch.add(hf,hb) for hf, hb in zip(h_forward, h_backward)]
        #print(obs1.shape)
