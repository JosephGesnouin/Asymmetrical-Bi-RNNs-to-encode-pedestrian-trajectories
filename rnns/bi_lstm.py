...
class LSTM(torch.nn.Module):
    def __init__(self, embedding_dim=64, hidden_dim=128, pool=None, pool_to_input=True, goal_dim=None, goal_flag=False):
        ...
        self.decoder = torch.nn.LSTMCell(self.embedding_dim + pooling_dim, 2*self.hidden_dim)
    ...
    def forward(self, observed, goals, batch_split, prediction_truth=None, n_predict=None):
        ...
        for obs1, obs2 in zip(inv_observed[1:], inv_observed[:-1]):
            ##LSTM Step
            h_backward = self.step(self.encoder1, h_backward, obs1, obs2, goals, batch_split, 
        ...
        for obs1, obs2 in zip(observed[:-1], observed[1:]):
            ##LSTM Step
            h_forward = self.step(self.encoder2, h_forward, obs1, obs2, goals, batch_split, 
        ...
        hidden_state = [torch.cat((hf,hb)) for hf, hb in zip(h_forward[0], h_backward[0])]
        hidden_state = (hidden_state,
                        [torch.zeros(2*self.hidden_dim, device=observed.device) 
                            for _ in range(num_tracks)])
