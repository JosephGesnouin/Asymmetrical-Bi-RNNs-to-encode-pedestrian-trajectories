...
from .lstm_original import LSTMPredictor, drop_distant
...
class Trainer(object):
    ...
    def loop(self, train_scenes, val_scenes, train_goals, val_goals, out, epochs=35, start_epoch=0):
        ...
        wait=0
        min_val_loss=1e10
        run_name =  out.replace("OUTPUT_BLOCK/trajdata/", "").replace(".pkl", "")
        self.writer=SummaryWriter("runs/"+run_name)
        for epoch in range(start_epoch, epochs):
            self.train(train_scenes, train_goals, epoch)
            val_loss = self.val(val_scenes, val_goals, epoch)
            if val_loss < min_val_loss:
                min_val_loss = val_loss
                state = {'epoch': epoch, 'state_dict': self.model.state_dict(),
                         'optimizer': self.optimizer.state_dict(),
                         'scheduler': self.lr_scheduler.state_dict()}
                LSTMPredictor(self.model).save(state, out + '.minval')
                wait=0
            else :
                wait+=1
            if wait>=patience:
    ...
    def train(self, scenes, goals, epoch):
        ...
        self.writer.add_scalar("Loss/train", epoch_loss / (len(scenes)), epoch+1)
    ...
    def val(self, scenes, goals, epoch):
        ...
        val_loss_norm = val_loss / len(scenes)
        self.lr_scheduler.step(val_loss_norm)
        self.writer.add_scalar("Loss/val", val_loss_norm, epoch+1)  
...
def main(epochs=25):
    ...
    parser.add_argument('--arch', default="lstm_original", type=str,
    ...
    mod = importlib.import_module('.'+args.arch, 'trajnetbaselines.rnns')
    LSTM = getattr(mod, "LSTM")
